# data-purge Specification

## Purpose

Provide one authorized, complete deletion path for all Markaroo data — DB tables, options, transients, user meta, cron events, and tagged media — reusing the same routines as `uninstall.php` so the two can never drift, and firing a Pro extension hook so the Pro plugin can purge alongside.
