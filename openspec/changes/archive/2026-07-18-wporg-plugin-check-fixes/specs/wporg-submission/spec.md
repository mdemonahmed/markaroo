...
- **THEN** it contains no `.DS_Store`, no dotfiles (`.gitignore`, `.editorconfig`, `.prettierrc`, `.eslintignore`, `.stylelintignore`, `.php-cs-fixer.cache`, `.gitattributes`), no `.claude/`, `.agents/`, `graphify-out/`, `.codegraph/`, or `openspec/` directories
