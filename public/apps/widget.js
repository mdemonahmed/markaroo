  ...
  try {
    canvas = await h2c(document.body, {
      ...
      onclone: clonedDoc => {
        ...
        if (clonedRoot) clonedRoot.style.display = 'none';
        if (opts.maskInputs) maskClone(clonedDoc);
      }
    });
  } finally {
...
async function uploadScreenshot(feedbackId, blob) {
  ...
  if (config.shareToken) headers['X-Markaroo-Share'] = config.shareToken;
  try {
...
function PinLayer() {
  ...
  (0,_wordpress_element__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    if (loadedRef.current || mode === 'clean') return;
  ...
  if (mode === 'clean' || feedbacks.length === 0) return null;
...
function ReplyComposer({
  ...
  async function handleSubmit(e) {
    ...
    if (!text.trim()) return;
    if (isGuest) {
      if (typeof localStorage !== 'undefined') localStorage.setItem(GUEST_NAME_KEY, guestName.trim());
    }
    setLoading(true);
    setError(null);
    try {
