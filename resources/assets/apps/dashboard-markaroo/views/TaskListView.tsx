...
const PRIORITY_COLORS: Record< string, string > = {
	urgent: '#ef4444', high: '#f97316', normal: '#6366f1', low: '#9ca3af',
};
...
export function TaskListView() {
	...
	const [ filters, setFilters ] = useState< Filters >( {
		status: 'open', priority: '', search: '', order_by: 'created_at', order: 'DESC', page: 1,
	} );
	...
	function SortButton( { col, label }: { col: string; label: string } ) {
		...
		return (
			...
			>
				{ label }{ active ? ( filters.order === 'DESC' ? ' ↓' : ' ↑' ) : '' }
			</button>
		);
	}
	...
	return (
		<div className="markaroo-admin-tasklist">
			...
			<table className="markaroo-admin-table markaroo-admin-tasklist__table">
				<thead>
					<tr>
						<th><SortButton col="created_at" label="#" /></th>
						<th>Comment</th>
						<th><SortButton col="status"   label="Status"   /></th>
						<th><SortButton col="priority" label="Priority" /></th>
						<th>Assignee</th>
						<th><SortButton col="due_date"   label="Due"     /></th>
						<th><SortButton col="created_at" label="Created" /></th>
						<th>Page</th>
					</tr>
				</thead>
				<tbody>
					...
					{ items.map( ( item ) => (
						<tr key={ item.id }>
							...
							<td>
								{ item.due_date
									? <span className={ new Date( item.due_date ) < new Date() && item.status === 'open' ? 'markaroo-admin-overdue' : '' }>
							...
							<td><code className="markaroo-admin-page-key">{ item.page_key }</code></td>
						</tr>
					) ) }
				</tbody>
			</table>
