...
export function ShareLinksView() {
	...
	useEffect( () => {
		fetch( restBase + 'shares', { headers: { 'X-WP-Nonce': config.nonce } } )
			.then( ( r ) => r.ok ? r.json() as Promise< ShareRow[] > : Promise.reject( r.status ) )
			.then( setShares )
	...
	async function createShare( e: React.FormEvent ) {
		...
		try {
			...
			const res = await fetch( restBase + 'shares', { method: 'POST', headers: headers(), body: JSON.stringify( body ) } );
		...
		<div className="markaroo-admin-shares">
			...
			{ showForm && (
				<form className="markaroo-share-form" onSubmit={ createShare }>
					<label>
						Label (internal)
						<input type="text" value={ label } onChange={ ( e ) => setLabel( e.target.value ) } placeholder="e.g. Client review" />
					</label>
					...
					{ scope === 'page' && (
						<label>
							Page path
							<input type="text" value={ pageKey } onChange={ ( e ) => setPageKey( e.target.value ) } placeholder="/my-page" />
						</label>
					) }
					...
					<button className="markaroo-admin-btn markaroo-admin-btn--primary" type="submit" disabled={ creating }>
						{ creating ? 'Creating…' : 'Create link' }
					</button>
				</form>
			) }

			{ shares.length === 0 && ! showForm && <p className="markaroo-admin__empty">No share links yet.</p> }
