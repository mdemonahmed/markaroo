    ...
    <div className="markaroo-reply">
      ...
      {canEdit && !editing && (
        <div className="markaroo-reply__actions">
          <button className="markaroo-reply__action" type="button" onClick={() => setEditing(true)}>
            Edit
          </button>
          {!confirm ? (
            <button
...
export function ThreadView({ feedback, onClose }: Props) {
  ...
  async function saveComment() {
    try {
      const updated = await apiPatch<FeedbackItem>(`feedback/${item.id}`, {
  ...
  return (
    <div className="markaroo-thread">
      <div className="markaroo-thread__header">
        <button className="markaroo-thread__back" type="button" onClick={onClose} aria-label="Back">
          ←
        </button>
        <span className="markaroo-thread__title">Feedback #{item.id}</span>
        {canResolve && (
          <button
            className={`markaroo-btn markaroo-btn--sm ${
              item.status === 'resolved' ? 'markaroo-btn--ghost' : 'markaroo-btn--primary'
            }`}
            type="button"
        ...
        <div className="markaroo-thread__replies">
          ...
          {replies.map((r) => (
            <ReplyRow
              ...
              onUpdated={(updated) =>
                setReplies((prev) => prev.map((x) => (x.id === updated.id ? updated : x)))
              }
