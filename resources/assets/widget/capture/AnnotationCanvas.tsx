...
import { renderAnnotationItem, renderAnnotations, canvasPct, burnAnnotationsIntoBlob } from './annotationUtils';
...
export function AnnotationCanvas( { screenshotBlob, onDone, onCancel }: AnnotationCanvasProps ) {
	...
	function handlePointerMove( e: React.PointerEvent< HTMLCanvasElement > ) {
		if ( ! drawingRef.current || ! canvasRef.current || ! currentRef.current ) return;
		const { xPct, yPct } = canvasPct( e.clientX, e.clientY, canvasRef.current );
	...
	async function handleDone() {
		...
		const opts = ( window.markarooConfig as Record< string, unknown > )
			?.screenshotOptions as { format?: 'jpeg' | 'png'; quality?: number } | undefined;
	...
	const availableTools = (
		( ( window.markarooConfig as Record< string, unknown > )?.annotationTools as Tool[] ) ?? [ 'arrow', 'rect', 'circle' ]
	...
	return (
		<div className="markaroo-annotation-wrap">
			...
			<div className={ `markaroo-annotation-stage${ bgLoaded ? ' markaroo-annotation-stage--ready' : '' }` }>
				<canvas
					ref={ canvasRef }
