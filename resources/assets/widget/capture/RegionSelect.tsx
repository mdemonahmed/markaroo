...
type Phase =
	...
	| { name: 'resizing'; rect: DrawRect; handle: Handle; startX: number; startY: number; origRect: DrawRect };
...
export function RegionSelect( { onCapture }: RegionSelectProps ) {
	...
	const handleOverlayPointerDown = useCallback(
		( e: React.PointerEvent< HTMLDivElement > ) => {
			if ( ( e.target as Element ).closest( '.markaroo-capture-toolbar,.markaroo-region-handle,.markaroo-region-confirm' ) ) {
				return;
			}
		...
		( e: React.PointerEvent< HTMLDivElement > ) => {
			setPhase( ( prev ) => {
				if ( prev.name === 'drawing' ) {
					...
					if ( rect.width < MIN_SIZE || rect.height < MIN_SIZE ) return { name: 'idle' };
	...
	function startResize( e: React.PointerEvent< HTMLDivElement >, handle: Handle, rect: DrawRect ) {
		e.stopPropagation();
		e.currentTarget.setPointerCapture( e.pointerId );
		setPhase( { name: 'resizing', rect, handle, startX: e.clientX, startY: e.clientY, origRect: rect } );
	}
